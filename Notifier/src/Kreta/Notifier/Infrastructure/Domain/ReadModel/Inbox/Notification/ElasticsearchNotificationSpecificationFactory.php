...
final class ElasticsearchNotificationSpecificationFactory implements NotificationSpecificationFactory
{
    public function createNotificationsOfUser(string $userId, int $from = 0, int $size = -1, string $status = null)
    {
        return new ElasticsearchNotificationsOfUserSpecification($userId, $from, $size, $status);
    }
}
